...
function App() {
  ...
  useEffect(() => {
    ...
    ]).then(([contentsData, sourcesData, detailData, seriesGenresData]) => {
  ...
  // Spanish genre label → English genre name substrings in the data
  const GENRE_MAP = {
    'Acción': ['action'],
    'Drama': ['drama'],
    'Comedia': ['comedy'],
  ...
  const availableLanguages = useMemo(() => {
    const set = new Set();
    movies.forEach(m => (m.languages || []).forEach(l => l && set.add(l.toLowerCase())));
    return [...set].sort();
  }, [movies]);
  ...
  const displayMovies = useMemo(() => {
    ...
    if (filterYear) {
      if (filterYear === 'Antes de 2010') result = result.filter(m => Number(m.releaseDate?.slice(0,4)) < 2010);
      else if (filterYear.includes('–')) {
        const [from, to] = filterYear.split('–').map(Number);
        result = result.filter(m => { const y = Number(m.releaseDate?.slice(0,4)); return y >= from && y <= to; });
      } else result = result.filter(m => m.releaseDate?.startsWith(filterYear));
    }
    if (filterDuration) {
      result = result.filter(m => {
        const rt = Number(m.runtime);
        if (!rt) return false;
        if (filterDuration === 'short') return rt < 90;
        if (filterDuration === 'medium') return rt >= 90 && rt <= 120;
        if (filterDuration === 'long') return rt > 120;
        return true;
      });
    }
    if (filterLanguage) result = result.filter(m => (m.languages || []).some(l => l?.toLowerCase() === filterLanguage));

    return result;
  }, [sectionMovies, activeGenre, activeSection, filterYear, filterDuration, filterLanguage]);
    ...
    <div className="app-container">
      ...
      {activeSection !== 'Mi Lista' && activeSection !== 'Historial' && (
        <FilterBar
          ...
          onFilterDuration={setFilterDuration}
          onFilterLanguage={setFilterLanguage}
          activeSection={activeSection}
        />
      )}

      <main style={{ paddingTop: '68px' }}>
        {/* Hero only on Inicio/Películas/Series, not when searching or in Mi Lista/Historial */}
        {!trimmedSearch && activeSection !== 'Mi Lista' && activeSection !== 'Historial' && (
