...
export default function FilterBar({ filterYear, filterDuration, filterLanguage, languages, onFilterYear, onFilterDuration, onFilterLanguage, activeSection }) {
  ...
  return (
    <div className="filter-bar container">
      ...
      {languages.length > 0 && (
        <select className="filter-select" value={filterLanguage} onChange={e => onFilterLanguage(e.target.value)}>
              <option value="">Cualquier región</option>
          {languages.map(l => <option key={l} value={l}>{l.toUpperCase()}</option>)}
        </select>
      )}
      {(filterYear || filterDuration || filterLanguage) && (
        <button className="filter-clear" onClick={() => { onFilterYear(''); onFilterDuration(''); onFilterLanguage(''); }}>
          × Limpiar filtros
        </button>
      )}
    </div>
